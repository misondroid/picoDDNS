...
def get_defaults():
    return {
        ...
        'password': 'pa55word!',
        'backend': 'floci' or env.get('COGNITO_NAME', 'floci'),
...
def get_help_descriptions():
    return {
        ...
        'backend': '作成先のCognitoバックエンド (デフォルト:floci, aws: AWS上)',
        'email': '管理者ユーザーのEmail',
...
def setup_cognito_user_pool(args):
    ...
    try:
        ...
        print(f'User pool client created:')
        pprint(user_pool_client)
    except Exception as e:
        raise e
...
def create_user_cognito(args,user_pool):
    secret_hash = SecretHashService(
        email=args.email,
        client_id=user_pool.get('ClientId'),
        client_secret=user_pool.get('ClientSecret')
    ).get()
    cognito = get_cognito_client(args)
    ...
    ユーザー作成の流れ:
    - flociの場合:
      - admin_create_userを使用してユーザーを作成
      - admin_set_user_passwordを使用してパスワードを設定し、ユーザーを有効化
    - kumoの場合:
      - sign_upを使用してユーザーを作成
      - confirm_sign_upを使用してユーザーを有効化(認証コードは固定なので定数で管理)
    """
    if args.backend == 'floci':
        try:
            user = cognito.admin_create_user(
                UserPoolId=user_pool.get('UserPoolId'),
        ...
        try:
            user = cognito.sign_up(
                ClientId=user_pool.get('ClientId'),
    ...
    user['UserAttributes'] = convert_user_attributes(user['UserAttributes'])
    return user
...
def main():
    ...
    pprint(user)
    print('APIキー作成開始')
    user_id = user.get('UserAttributes').get('sub')
    api_key = register_api_key(user_id=user_id,email=args.email)
    print('APIキー作成完了')
    print('APIキー情報:')
    pprint(api_key._as_dict())
    print('ユーザー登録開始')
    user = register_user(user_id=user_id,email=args.email)
    print('ユーザー登録完了')
    print('ユーザー情報:')
